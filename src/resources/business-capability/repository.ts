...
const mapBusinessCapability = (row: any): BusinessCapability => ({
  id: Number(row.id), businessId: Number(row.business_id), capabilityId: String(row.capability_id),
  proficiencyLevel: row.proficiency_level, description: row.description, assertionStatus: row.assertion_status,
  assertionBasis: row.assertion_basis, verificationStatus: row.verification_status, effectiveFrom: row.effective_from,
  effectiveUntil: row.effective_until, sourceReference: row.source_reference, submittedAt: row.submitted_at,
  verifiedBy: row.verified_by === null ? null : Number(row.verified_by), verifiedAt: row.verified_at,
  verificationReason: row.verification_reason, createdBy: row.created_by === null ? null : Number(row.created_by),
    ...
    return withAuthorizedTransaction(context, async client => {
      ...
      const result = await client.query(
        `INSERT INTO ghm.business_capability
          (business_id, capability_id, proficiency_level, description, effective_from, effective_until, source_reference, created_by)
         VALUES ($1, $2, $3, NULLIF(btrim($4), ''), COALESCE($5, now()), $6, NULLIF(btrim($7), ''), $8)
         RETURNING ${COLUMNS}`,
        [businessId, capabilityId, input.proficiencyLevel ?? null, input.description ?? null, input.effectiveFrom ?? null,
          input.effectiveUntil ?? null, input.sourceReference ?? null, context.userId],
      );
