...
const mapAccount = (row: any): AccountIdentity => ({
  id: Number(row.id), fullName: row.full_name, phone: row.phone, avatarRef: row.avatar_ref,
  role: row.role, createdAt: row.created_at, updatedAt: row.updated_at,
});
const mapBusiness = (row: any): BusinessIdentity => ({
  id: Number(row.id), name: row.name, slug: row.slug, verificationStatus: row.verification_status,
  isActive: row.is_active, createdAt: row.created_at, updatedAt: row.updated_at,
});
const mapMembership = (row: any): BusinessMembership => ({
  id: Number(row.id), businessId: Number(row.business_id), accountId: Number(row.account_id),
...
const findBusiness = async (client: PoolClient, businessId: BusinessId) => {
  const result = await client.query(BUSINESS_SELECT, [businessId]);
  return result.rowCount === 1 ? mapBusiness(result.rows[0]) : null;
};
const findBusinessBySlug = async (client: PoolClient, slug: string) => {
  const result = await client.query(BUSINESS_BY_SLUG_SELECT, [slug]);
  return result.rowCount === 1 ? mapBusiness(result.rows[0]) : null;
};
const findMemberships = async (client: PoolClient, accountId: number) => {
...
const assertManagedMembership = async (client: PoolClient, context: AuthContext, businessId: BusinessId) => {
  const result = await client.query(
    `SELECT 1 FROM business_membership WHERE business_id = $1 AND account_id = $2
      AND membership_status = 'active' AND membership_role IN ('owner', 'administrator') LIMIT 1`,
...
export class PostgresBusinessIdentityRepository implements BusinessIdentityRepository {
  ...
  async updateAccount(context: AuthContext, input: UpdateProfileInput) {
    return withAuthorizedTransaction(context, async client => {
      ...
      const result = await client.query(`UPDATE account_identity
        ...
        RETURNING id, full_name, phone, avatar_ref, role, created_at, updated_at`, [
          context.userId, Object.hasOwn(input, 'fullName'), input.fullName ?? null,
          Object.hasOwn(input, 'phone'), input.phone ?? null,
          Object.hasOwn(input, 'avatarRef'), input.avatarRef ?? null,
        ]);
      if (result.rowCount !== 1) throw new Error('Authenticated account not found');
      return mapAccount(result.rows[0]);
  ...
  async createBusiness(context: AuthContext, input: CreateBusinessInput, slug: string) {
    return withAuthorizedTransaction(context, async client => {
      ...
      const businessResult = await client.query(`INSERT INTO business (name, slug, verification_status, is_active)
        VALUES ($1, $2, 'pending', true) RETURNING id, name, slug, verification_status, is_active, created_at, updated_at`, [normalizeName(input.name), slug]);
      const business = mapBusiness(businessResult.rows[0]);
      await client.query(`INSERT INTO business_membership
        (business_id, account_id, membership_role, membership_status, created_by)
        VALUES ($1, $2, 'owner', 'active', $2)`, [business.id, context.userId]);
      return business;
    }, this.transactionPool);
  }
  async updateBusiness(context: AuthContext, businessId: BusinessId, input: UpdateBusinessProfileInput) {
    ...
    return withAuthorizedTransaction(context, async client => {
      ...
      const result = await client.query(`UPDATE business
        SET name = CASE WHEN $2 THEN $3 ELSE name END,
            slug = CASE WHEN $4 THEN $5 ELSE slug END,
            updated_at = now() WHERE id = $1
        RETURNING id, name, slug, verification_status, is_active, created_at, updated_at`, [
          businessId, Object.hasOwn(input, 'name'), Object.hasOwn(input, 'name') ? normalizeName(input.name!) : null,
          Object.hasOwn(input, 'slug'), Object.hasOwn(input, 'slug') ? input.slug!.trim() : null,
        ]);
