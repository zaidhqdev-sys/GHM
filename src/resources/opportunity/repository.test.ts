...
class FakeClient {
  ...
  async queryTransaction(text: string) {
    if (text === 'BEGIN') return this.query(text);
    if (text === 'COMMIT') { this.committed = true; return this.query(text); }
    if (text === 'ROLLBACK') { this.rolledBack = true; return this.query(text); }
...
const patchTransactionCommands = (client: FakeClient) => {
  ...
  client.query = async (text: string, values?: unknown[]) => {
    if (text === 'BEGIN') { client.queries.push({ text, values }); return { rowCount: null, rows: [] }; }
    if (text === 'COMMIT') { client.committed = true; client.queries.push({ text, values }); return { rowCount: null, rows: [] }; }
    if (text === 'ROLLBACK') { client.rolledBack = true; client.queries.push({ text, values }); return { rowCount: null, rows: [] }; }
...
test('Opportunity creation atomically establishes creator participation', async () => {
  ...
  patchTransactionCommands(client);
  const repository = new PostgresOpportunityRepository(new FakePool(client));

  const result = await repository.createOpportunity(context, {
    opportunityTypeId: 1,
    title: 'Service request',
    description: 'A governed opportunity fixture.',
  });
  ...
  const participantQuery = client.queries.find((query) => query.text.includes('INSERT INTO ghm.opportunity_participant'));
  assert.ok(participantQuery);
  assert.match(participantQuery.text, /'creator','active'/);
  assert.deepEqual(participantQuery.values, [11, 7]);
});

test('Opportunity creation atomically establishes owner Business participation when supplied', async () => {
  const client = new FakeClient(23);
  patchTransactionCommands(client);
  const repository = new PostgresOpportunityRepository(new FakePool(client));
  ...
  await repository.createOpportunity(context, {
    opportunityTypeId: 1,
    ownerBusinessId: 23,
    title: 'Business-owned opportunity',
    description: 'A governed opportunity fixture.',
  });

  const participantQueries = client.queries.filter((query) => query.text.includes('INSERT INTO ghm.opportunity_participant'));
  assert.equal(participantQueries.length, 2);
  assert.match(participantQueries[0].text, /'creator','active'/);
  assert.deepEqual(participantQueries[0].values, [11, 7]);
  assert.match(participantQueries[1].text, /'owner','active'/);
  assert.deepEqual(participantQueries[1].values, [11, 23, 7]);
  assert.equal(client.committed, true);
});

test('Opportunity creation rolls back when creator participation cannot be established', async () => {
  const client = new FakeClient(null, true);
  patchTransactionCommands(client);
  const repository = new PostgresOpportunityRepository(new FakePool(client));

  await assert.rejects(
    () => repository.createOpportunity(context, {
      opportunityTypeId: 1,
      title: 'Rollback opportunity',
      description: 'The participant write must be atomic.',
    }),
...
test('Opportunity creation rolls back when owner participation cannot be established', async () => {
  const client = new FakeClient(23, true);
  patchTransactionCommands(client);
  const repository = new PostgresOpportunityRepository(new FakePool(client));
  ...
  await assert.rejects(
    () => repository.createOpportunity(context, {
      opportunityTypeId: 1,
      ownerBusinessId: 23,
      title: 'Rollback owner opportunity',
      description: 'The owner participant write must be atomic.',
    }),
    /participant insert failed/,
  );
