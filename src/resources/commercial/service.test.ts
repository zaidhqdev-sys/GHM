...
const access = (): CommercialAccess => ({
  ...
  entitlements: [{ code: 'quote_management', accessLevel: 'enabled', metadata: {} }],
...
class FakeRepository implements CommercialRepository {
  ...
  trialCalls: Array<{ context: AuthContext; input: ActivateCommercialTrialInput }> = [];
  paymentCalls: Array<{ context: AuthContext; input: PrepareCommercialPaymentInput }> = [];
  cancellationCalls: Array<{ context: AuthContext; input: ScheduleCommercialCancellationInput }> = [];

  async getCommercialAccess(context: AuthContext, businessId: number): Promise<CommercialAccess> {
    this.accessCalls.push({ context, businessId });
    return access();
  }

  async getCommercialSubscription(context: AuthContext, businessId: number): Promise<CommercialSubscription | null> {
    this.subscriptionCalls.push({ context, businessId });
    return subscription();
  }

  async activateCommercialTrial(context: AuthContext, input: ActivateCommercialTrialInput): Promise<CommercialTrial> {
  ...
  async prepareCommercialPayment(context: AuthContext, input: PrepareCommercialPaymentInput): Promise<CommercialPaymentAttempt> {
  ...
  async scheduleCommercialCancellation(context: AuthContext, input: ScheduleCommercialCancellationInput): Promise<CommercialSubscription> {
...
test('Commercial service delegates Business-scoped access reads', async () => {
  ...
  assert.deepEqual(repository.accessCalls, [{ context, businessId: 11 }]);
});

test('Commercial service delegates trial activation including explicit plan code', async () => {
  const repository = new FakeRepository();
  const service = new DefaultCommercialService(repository);
  const input: ActivateCommercialTrialInput = { businessId: 11, planCode: 'business_pro' };
  ...
  assert.deepEqual(repository.trialCalls, [{ context, input }]);
});

test('Commercial service delegates payment preparation', async () => {
  const repository = new FakeRepository();
  const service = new DefaultCommercialService(repository);
  const input: PrepareCommercialPaymentInput = { businessId: 11, idempotencyKey: 'test-idempotency-key', expiresAt: new Date(5000) };
  const result = await service.prepareCommercialPayment(context, input);
  assert.equal(result.id, 61);
  assert.equal(result.businessId, 11);
  assert.deepEqual(repository.paymentCalls, [{ context, input }]);
});

test('Commercial service delegates subscription cancellation', async () => {
  const repository = new FakeRepository();
  const service = new DefaultCommercialService(repository);
  const input: ScheduleCommercialCancellationInput = { businessId: 11, subscriptionId: 21, reason: 'Customer requested cancellation' };
  const result = await service.scheduleCommercialCancellation(context, input);
  assert.equal(result.id, 21);
  assert.equal(result.businessId, 11);
  assert.deepEqual(repository.cancellationCalls, [{ context, input }]);
});
...
test('Commercial service delegates Business-scoped subscription reads', async () => {
  ...
  assert.deepEqual(repository.subscriptionCalls, [{ context, businessId: 11 }]);
});
