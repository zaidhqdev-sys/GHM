...
try {
  ...
  const privilegeChecks = await runtimePool.query(
    `SELECT has_table_privilege(current_user, 'ghm.business_capability', 'SELECT') AS can_select,
  ...
  if (
    ...
    privilegeColumns.some((column) => !privileges[`insert_${column}`]) ||
    protectedColumns.some((column) => privileges[`protected_insert_${column}`])
  ) {
