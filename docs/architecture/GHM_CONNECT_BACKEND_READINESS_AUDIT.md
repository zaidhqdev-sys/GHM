...
| 14 | App tables without migration evidence | Services reference tables not created in audited migrations | Disputes, referrals, workspace, suppliers, admin queue, etc. | Unknown | Cannot define GHM contract safely from client fiction alone; class A domains are separate (see referenced-object reconciliation) | Prove or retire E/F objects; authorize class A domains separately | Yes before construction |
