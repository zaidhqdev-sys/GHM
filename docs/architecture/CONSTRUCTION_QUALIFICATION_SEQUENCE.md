...
4. **Resource API** — unrestricted generic table access is removed and replaced by explicit governed resources. **Contract primitives exist; product-resource implementation/qualification remains open.**
...
7. **Database reconciliation** — actual GHM PostgreSQL catalog is captured and reconciled before dependent product/business migrations are authored. **Catalog capture, first-slice schema reconciliation, runtime authority qualification, and dedicated migrator qualification are complete for the current slice. Remaining work includes bootstrap membership cleanup, dedicated-schema/default-privilege reconciliation, and subsequent governed resource slices.**
